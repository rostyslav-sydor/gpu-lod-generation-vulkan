...
void App::runDecimation() {
    ...
    for (uint32_t iteration = 0; iteration < maxDecimationIterations; iteration++) {
        ...
        // Normal (batched) path
        if (isLight) {
            // Leave COLLAPSE_COUNT intact — P3 thread 0 reads & clears it to update TRIANGLE_COUNT.
            // Double-counting after full iterations is prevented by clearing COLLAPSE_COUNT
            // after compaction (see below).
...
void App::stepInteractiveDecimation() {
    ...
    if (isLight) {
        // Leave COLLAPSE_COUNT intact — P3 reads & clears it to update TRIANGLE_COUNT.
