...
void App::cleanup() {
    ...
    if (flatPipeline != VK_NULL_HANDLE) vkDestroyPipeline(device, flatPipeline, nullptr);
    if (wireframePipeline != VK_NULL_HANDLE) vkDestroyPipeline(device, wireframePipeline, nullptr);
    if (edgeLinePipeline != VK_NULL_HANDLE) vkDestroyPipeline(device, edgeLinePipeline, nullptr);
    ...
    vkDestroyBuffer(device, compMeshletTrianglesBuffer, nullptr);
    vkDestroyBuffer(device, compMeshletVerticesBuffer, nullptr);

    vkFreeMemory(device, compVertexBufferMemory, nullptr);
    vkFreeMemory(device, compMeshletsMemory, nullptr);
    vkFreeMemory(device, compMeshletTrianglesMemory, nullptr);
    vkFreeMemory(device, compMeshletVerticesMemory, nullptr);
...
void App::createInstance() {
    ...
    appInfo.pApplicationName = "GPU LOD WHATEVER";
...
void App::createGraphicsPipeline() {
    ...
    // --- Flat debug + wireframe pipelines ---
    auto flatVertCode = readFile("shaders/vert_flat.spv");
    auto flatFragCode = readFile("shaders/frag_flat.spv");
    VkShaderModule flatVertModule = createShaderModule(flatVertCode);
    VkShaderModule flatFragModule = createShaderModule(flatFragCode);

    VkPipelineShaderStageCreateInfo flatVertStage{};
    flatVertStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    flatVertStage.stage = VK_SHADER_STAGE_VERTEX_BIT;
    flatVertStage.module = flatVertModule;
    flatVertStage.pName = "main";

    VkPipelineShaderStageCreateInfo flatFragStage{};
    flatFragStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    flatFragStage.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    flatFragStage.module = flatFragModule;
    flatFragStage.pName = "main";

    VkPipelineShaderStageCreateInfo flatStages[] = { flatVertStage, flatFragStage };

    pipelineInfo.pStages = flatStages;

    rasterizer.cullMode = VK_CULL_MODE_NONE;
    rasterizer.polygonMode = VK_POLYGON_MODE_FILL;
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &flatPipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create flat pipeline!");
    }
    ...
    // Edge line pipeline: LINE_LIST topology for per-edge colored lines
    inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
    rasterizer.polygonMode = VK_POLYGON_MODE_FILL;
    rasterizer.lineWidth = 2.0f;
    rasterizer.depthBiasConstantFactor = -2.0f;
    rasterizer.depthBiasSlopeFactor = -2.0f;
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &edgeLinePipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create edge line pipeline!");
    }
    inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;

    vkDestroyShaderModule(device, flatFragModule, nullptr);
    vkDestroyShaderModule(device, flatVertModule, nullptr);
}
...
void App::createVertexBuffer() {
    ...
    createAndCopyBufferLocal(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertices.data(), vertexBuffer, vertexBufferMemory);
}
...
void App::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_F) {
        app->flatDebugMode = !app->flatDebugMode;
        if (app->flatDebugMode) {
            app->buildFlatDebugMesh();
        }
        std::cout << "Flat debug mode: " << (app->flatDebugMode ? "ON" : "OFF") << std::endl;
    }
}
...
void App::scrollCallback(GLFWwindow* window, double /*xoffset*/, double yoffset) {
    auto app = reinterpret_cast<App*>(glfwGetWindowUserPointer(window));
    float factor = (yoffset > 0) ? 1.25f : 0.8f;
    app->cameraSpeed = glm::clamp(app->cameraSpeed * factor, 0.01f, 100.0f);
...
void App::copyBuffer2(VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkDeviceSize srcOff = 0, VkDeviceSize dstOff = 0) {
    VkBufferCopy copyRegion{};
    copyRegion.size = size;
...
void App::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (flatDebugMode && flatDebugSnap.valid && flatDebugSnap.vertBuf != VK_NULL_HANDLE) {
        VkBuffer vertexBuffers[] = { flatDebugSnap.vertBuf };
        VkDeviceSize offsets[] = { 0 };
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
        vkCmdBindIndexBuffer(commandBuffer, flatDebugSnap.idxBuf, 0, VK_INDEX_TYPE_UINT32);
        uint32_t indexCount = static_cast<uint32_t>(flatDebugSnap.inds.size());

        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, flatPipeline);
        vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
        vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
        uint32_t fillMode = 1u;
        vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(uint32_t), &fillMode);
        vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);

        if (edgeLineBuf != VK_NULL_HANDLE && edgeLineVertCount > 0) {
            vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, edgeLinePipeline);
            vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
            vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
            uint32_t edgeMode = 0u;
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(uint32_t), &edgeMode);
            VkBuffer edgeBuffers[] = { edgeLineBuf };
            VkDeviceSize edgeOffsets[] = { 0 };
            vkCmdBindVertexBuffers(commandBuffer, 0, 1, edgeBuffers, edgeOffsets);
            vkCmdDraw(commandBuffer, edgeLineVertCount, 1, 0, 0);
        }
    } else {
...
void App::updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.proj = glm::perspective(glm::radians(45.0f), swapChainExtent.width / (float)swapChainExtent.height, 0.001f, 500.0f);
    ubo.proj[1][1] *= -1;
