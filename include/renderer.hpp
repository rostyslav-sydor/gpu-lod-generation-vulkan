...
inline bool headlessMode = (std::getenv("HEADLESS") && std::string(std::getenv("HEADLESS")) != "0");
inline std::string experimentTag = std::getenv("EXP_TAG") ? std::getenv("EXP_TAG") : "";
...
private:
    ...
    VkPipeline flatPipeline = VK_NULL_HANDLE;
    VkPipeline wireframePipeline = VK_NULL_HANDLE;
    VkPipeline edgeLinePipeline = VK_NULL_HANDLE;
    ...
    // Heat map visualization (0=off, 1=curvature, 2=change map)
    int heatMapMode = 0;
    bool heatMapEnabled = false;
    std::vector<glm::vec3> savedNormals;
    std::vector<glm::vec3> prevPositions;
    void computeHeatMapColors();
    void computeChangeMap();

    // Flat debug render mode: per-triangle colors + wireframe edges
    bool flatDebugMode = false;
    MeshSnapshot flatDebugSnap;
    VkBuffer edgeLineBuf = VK_NULL_HANDLE;
    VkDeviceMemory edgeLineMem = VK_NULL_HANDLE;
    uint32_t edgeLineVertCount = 0;
    void buildFlatDebugMesh();

    // Per-step GPU state readback for scatter-fight visualization
    // stepPhase: 0 = ready for scatter, 1 = scatter done (showing scatter fight),
    //            2 = collapse done (showing result)
    int stepPhase = 0;
    struct StepVisData {
        std::vector<uint32_t> edges;        // flat [v0,v1, v0,v1, ...] (posIndex pairs)
        std::vector<uint32_t> edgeFlags;
        std::vector<float>    edgeCost;
        std::vector<uint32_t> vertexFlags;
        std::vector<uint32_t> posIndices;   // posIndices for triangles
        std::vector<uint64_t> triDescriptor; // per-triangle winning edge descriptor (scatter-phase indices)
        uint32_t edgeCount = 0;
        uint32_t triCount = 0;             // scatter-phase tri count (pre-compaction)
        bool valid = false;
    } stepVis;
    void stepInteractiveScatter();
    void stepInteractiveCollapse();
